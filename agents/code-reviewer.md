...
4. The project's CLAUDE.md — for framework-specific rules (KnockOff, Neatoo, RemoteFactory, EF Core, etc.) that apply to Category 5 (Framework Correctness)
...
5. **Framework Correctness** — Check the project's CLAUDE.md framework rules against the modified code. For zTreatment: Neatoo/RemoteFactory/KnockOff idioms, `.ToUtcForDb()` on DateTime saves, no swallowed exceptions, no reflection, no interface-to-concrete casts, domain logic in domain models not Blazor.
...
7. **Scope Discipline** — Check the plan's Out of Scope / Invariants list against the actual changes. Anything on the list that was touched is a concern, unless a Design Decisions entry explicitly authorizes it.
