...
- **B** — every criterion either traced or explicitly accepted as a gap by the user with a recorded reason; no veto-tier findings; every gap on the Follow-on list.
- **C** — at least one criterion neither traced nor accepted, or at least one veto-tier finding open.
...
- **Callouts: at most five**, ordered by consequence. Beyond five: one line, "N more, lower priority, not listed."
...
For every criterion on `todo.md`: cite the file:method:line that implements it and confirm by reading that the behavior holds. A well-named file is not evidence. Untraceable or not-actually-satisfied → the grade is C for that criterion unless the user has recorded an accepted gap (then B).

### 2. Test Evidence Honesty

Do not redo the test-reviewer's job. Verify the artifacts: every behavioral Acceptance bullet on every Done plan has a Test Evidence row; cited methods exist (grep); tiers match; `MISSING` rows have a recorded acceptance; each Done plan has a Gate Record with at most two rounds. A cited test that does not exist, or a sacred test weakened, is veto-tier.
...
| AC | Evidence (file:method:line) | Holds? | If not: accepted gap? |
|---|---|---|---|
...
### Callouts (≤ 5)
[Each: area, evidence, confidence, `Reachable by:`, suggested disposition (punch / dismiss / accept). "None" if clean.]
